...
locals {
  runtime_config_hash = sha256(jsonencode({
    name          = "${var.agent_runtime_name}-${var.environment}"
...
resource "null_resource" "agentcore_runtime" {
  provisioner "local-exec" {
    command = <<-EOT
      ...
      EXISTING=$(aws bedrock-agentcore-control list-agent-runtimes \
        ...
        --query "agentRuntimeSummaries[?agentRuntimeName=='${var.agent_runtime_name}-${var.environment}'].agentRuntimeId" \
      ...
      if [ -z "$EXISTING" ] || [ "$EXISTING" = "None" ]; then
        ...
        aws bedrock-agentcore-control create-agent-runtime \
          --region ${data.aws_region.current.name} \
          --agent-runtime-name "${var.agent_runtime_name}-${var.environment}" \
  ...
  triggers = {
    ...
    region       = data.aws_region.current.name
    runtime_name = "${var.agent_runtime_name}-${var.environment}"
  }
}
...
data "external" "agentcore_runtime_info" {
  ...
  program = ["bash", "-c", <<-EOT
    RUNTIME=$(aws bedrock-agentcore-control list-agent-runtimes \
      --region ${data.aws_region.current.name} \
      --query "agentRuntimeSummaries[?agentRuntimeName=='${var.agent_runtime_name}-${var.environment}'] | [0]" \
